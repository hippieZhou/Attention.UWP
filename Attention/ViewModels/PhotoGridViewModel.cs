...
namespace Attention.ViewModels
{
    public class PhotoGridViewModel : PixViewModel<PhotoItemSource, ImageItem>
    {
        protected override void OnLoaded()
        {
            if (Items == null)
            {
                Items = new IncrementalLoadingCollection<PhotoItemSource, ImageItem>(itemsPerPage: 20,
                     () =>
                    {
    ...
    public class PhotoItemSource : IIncrementalSource<ImageItem>
    {
        ...
        {
            ImageResult result = await ViewModelLocator.Current.Pixabay.QueryImagesAsync(ViewModelLocator.Current.AppSettings.Filters, page: ++pageIndex, per_page: pageSize);
    ...
    {
        public GridView View { get; protected set; }
